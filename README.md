...
- id = para saber que id tiene
- whoami = para saber en servidor estoy 
- sudo cat /etc/passwd = encontramos todos los usuarios del sistema operativo = ver todos los usuarioscreados
- ls -l /etc/shadow = con eso sabemos que privilegios tiene shadow
- sudo passwd root = estamos cambiando el password de el usuario root
- sudo useradd roberto = creamos un nuevo usuario
- history = nos muestra todo los comandos ya aplicados en el terminal
- sudo su  - jonathan -a sudo = con esto pasamos al usuario jonathan al grupo sudo
s- udo apt-get update = siempre actualizar cuando se actualiza algo

- PAM es un mecanismo para administrar a los usuarios de nuestro sistema operativo

- ls /etc/pam.d = las carpetas y archivos de la carpeta pam 

## ENTRAR A UN REPOSITORIO GITHUB
- cd = para movernos
- ls = ´para saber en que carpeta estamos
- clonamos el repositorio = github clone... pegamos el link del repositorio
- ojo:debemos estar en las carpetas
- ./cupp.py -i = ejecutamos el programa con el -i

## PARA TENER EL IP EN LINUX ES 
- sudo ifconfig ( inet 192.168.79.128)

## PARA CONECTAR LINUX CON EL POWERSHELL DE WINDOWS
- 1.-abrimos el powershell
- 2.- escribimos ssh codevars@192.168.1.155 password:kali

## MANEJO DE USUARIOS
- usermod --lock user -  se bloque un usuario
- ctrl + z salgo de los usuarios
